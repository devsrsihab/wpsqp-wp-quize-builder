...
// Helper function to get image URL from ID
function wpsqp_view_get_image_url($image_id) {
    if (empty($image_id)) {
        return '';
    }
...
<div class="wpsqp-view-normal-mcq-images">
    ...
    <?php if (!empty($option_content)): ?>
    <div class="view-section option-content-section">
        <div class="section-title">
            <span class="dashicons dashicons-editor-help"></span>
            <h3><?php _e('Question Statement', 'wpsqptxd'); ?></h3>
        </div>
        <div class="section-content option-content">
    ...
    <div class="view-section options-section">
        ...
        <div class="options-grid">
            <?php foreach ($options as $index => $option):
                ...
                $option_image_id = is_array($option) ? (isset($option['image_id']) ? intval($option['image_id']) : 0) : 0;
                $option_image_url = wpsqp_view_get_image_url($option_image_id);
                $is_correct = ($option_id === $correct_answer);
                $letter = chr(65 + $index);
                
                // Debug log
                error_log('Option ' . $index . ' - Image ID: ' . $option_image_id . ', URL: ' . $option_image_url);
            ?>
            <div class="option-card <?php echo $is_correct ? 'correct-option' : ''; ?>">
                ...
                <?php if (!empty($option_image_url)): ?>
                <div class="option-image">
                    <img src="<?php echo esc_url($option_image_url); ?>" alt="<?php echo esc_attr($option_text); ?>">
                </div>
                <?php else: ?>
